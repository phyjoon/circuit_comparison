...
class BaseObserver:
    ...
    def __init__(self):
        ...
        # relative small. Thus, for simplicity we here set baseline params
        # as an attribute.
        self.center = None
    ...
    def evaluate(self):
        if self.position is None:
            raise RuntimeError('Model params has not been set yet. '
    ...
    def plot(self,
             draw_trajectory=True, draw_contour=True, draw_colorbar=True,
             show=True, pause_interval=0., save_path=None):
        x, y, landscape = self.retrieve_landscape()

        if draw_trajectory:
            trajectory = visualizer.project_trajectory(
                self.x_axis, self.y_axis, self.trajectory)
        else:
            trajectory = None
...
class PyTorchModelObserver(BaseObserver):
    ...
    def setup(self, center=None, x_axis=None, y_axis=None,
              scale=math.pi, grid_size=10,
              trajectory_length=20, cache_type='numpy'):
        """ Setting up observing landscape

        # Args.
            center: iterable of Parameters, weight parameter
            x_axis: iterable of float tensors, a list of params that indicates
            ...
            y_axis: iterable of float tensors, a list of params that indicates
            ...
            grid_size: int or tuple of length 2, the size of observation grid.
                ...
                (grid_size, grid_size)
            trajectory_length: int, maximum length of trajectory.
                Parameters in `trajectory` will be used for drawing the trajectory
                of a training procedure.
            cache: str, cache type, caching intermediate results during traversing
    ...
    def set_weights_impl(self, i, j):
        ...
        for p, w, dx, dy in zip(self.model.parameters(), self.center,
...
class PennylaneModelObserver(BaseObserver):
    ...
    def setup(self, center=None, x_axis=None, y_axis=None,
        ...
        # Args.
            center: iterable of Parameters, weight parameter
            x_axis: numpy.ndarray, params that indicates the x-axis of the observation plane.
            y_axis: numpy.ndarray, params that indicates the y-axis of the observation plane.
            scale: float, scaling factor of direction vectors.
            grid_size: int or tuple of length 2, the size of observation grid.
                When a single int value is given, it will be interpreted as
                (grid_size, grid_size)
    ...
    def update_model_params(self, params):
        ...
        # projection to observation plane
