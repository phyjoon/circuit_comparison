...
def param_shape(circuit, size):
    param_dic = {
        ...
        5: size * (size + 3),
        6: size * (size + 3),
        7: 4 * size + floor(size / 2) + floor((size - 1) / 2),
        8: 4 * size + floor(size / 2) + floor((size - 1) / 2),
        9: size,
        10: 2 * size,
        11: 4 * size - 4,
        12: 4 * size - 4,
        ...
        14: 4 * size,
        15: 2 * size,
        16: 2 * size + floor(size / 2) + floor((size - 1) / 2),
        17: 2 * size + floor(size / 2) + floor((size - 1) / 2),
        18: 3 * size, 19: 3 * size
    }
...
def circuit05(params, wires):
    ...
    for cnt, controlled in enumerate(reversed(range(len(wires)))):
        ...
        qml.broadcast(unitary=qml.CRZ, pattern=pattern, wires=wires,
...
def circuit06(params, wires):
    ...
    for cnt, controlled in enumerate(reversed(range(len(wires)))):
        pattern = [[controlled, j] for j in reversed(range(len(wires))) if j != controlled]
        qml.broadcast(unitary=qml.CRX, pattern=pattern, wires=wires,
                      parameters=params[idx_start:idx_start + (size - 1)])
        idx_start += size - 1
